...
class ControllerTest {
    ...
    void testFireCooldownSetAfterHit() {
        fireAndWait(new FireAttack(), 15.0, 200);
        assertEquals(Controller.FIRE_COOLDOWN, controller.getFireCooldown(true));
    }
    ...
    void testIceCooldownSetAfterHit() {
        fireAndWait(new IceAttack(), 15.0, 200);
        assertEquals(Controller.ICE_COOLDOWN, controller.getIceCooldown(true));
    }

    @Test
    void testFrozenPlayerTurnIsAutoSkipped() {
        fireAndWait(new IceAttack(), 15.0, 200);
        // P2 is frozen; running updates should eventually skip P2's turn back to P1
    ...
    void testWindCooldownSetAfterHit() {
        fireAndWait(new WindAttack(), 15.0, 200);
        assertEquals(Controller.WIND_COOLDOWN, controller.getWindCooldown(true));
    }
    ...
    void testFireCooldownDecrementsAfterOpponentTurn() {
        fireAndWait(new FireAttack(), 15.0, 200);          // P1 hits → cooldown = 3
        int cdAfterHit = controller.getFireCooldown(true);
        fireAndWait(new NormalAttack(), Projectile.MIN_ANGLE, 300); // P2 misses → back to P1
        assertEquals(cdAfterHit - 1, controller.getFireCooldown(true));
    }

    @Test
    void testIceIsRejectedWhileOnCooldown() {
        fireAndWait(new IceAttack(), 15.0, 200);           // P1 hits → ice on cooldown
        // skip frozen P2 turn
        for (int i = 0; i < Controller.FROZEN_DISPLAY_FRAMES + 10; i++) controller.update();
        // P1 turn again — ice still on cooldown
        assertTrue(controller.getIceCooldown(true) > 0);
        controller.selectAttack(new IceAttack());          // rejected silently
        controller.handleSpacebar();                       // no attack selected → stays IDLE
        assertEquals(Projectile.State.IDLE, controller.getProjectile().getState());
    }
    ...
    void testDogAbilityCooldownSet() {
        controller.handleAbility();
        // switchTurn() decrements the NEW current player's cooldowns (P2), not P1's
    ...
    void testCatAbilityDoesNotSwitchTurn() {
        ...
        Controller c = new Controller(catP1, new DogPlayer());
        c.handleAbility();
        assertTrue(c.isPlayer1Turn()); // turn does NOT end for Cat ability
    }
